...
function plainTerrainTile(entry: MapPaletteEntry, tileX: number, tileY: number, size: number, onReady?: () => void, now?: number) {
  ...
  const frameKey = frameKeyFor(fps, now);
  const key = `${entry.id}|${size}|${flipX ? 1 : 0}${flipY ? 1 : 0}`;
  ...
  plainTileCache.set(key, { canvas, animated: fps > 0, frameKey, fps });
  trimCache(plainTileCache, MAX_PLAIN_CACHE);
  return canvas;
}
...
function drawWorldAlignedTerrain(
  ...
  const cacheSize = renderSizeFor(size);
  const cached = plainTerrainTile(entry, tileX, tileY, cacheSize, onReady, now);
  ctx.save();
  ctx.globalAlpha *= alpha;
  if (cached) {
    ctx.imageSmoothingEnabled = true;
    ctx.drawImage(cached, x, y, size + .7, size + .7);
  } else {
    const flipX = Math.abs(tileX) % 2 === 1;
    const flipY = Math.abs(tileY) % 2 === 1;
...
function cachedBlendTile(
  ...
  const size = renderSizeFor(options.tilePixels);
  const baseKey = `${map.id}|${layer}|${size}|${options.x},${options.y}|${fingerprint}`;
  const existing = blendedTileCache.get(baseKey);
  if (existing) {
    const frameKey = frameKeyFor(existing.fps, options.now);
    if (!existing.animated || existing.frameKey === frameKey) return existing.canvas;
  }

  const group = neighborhood(map, options.x, options.y, layer);
  if (group.ids.length <= 1) return null;

  const entries: MapPaletteEntry[] = [];
  let maxFps = 0;
  ...
  const masks = createMaskCanvases(group, options.x, options.y, maskResolution);
  ...
  for (let index = 0; index < entries.length; index++) {
    ...
    layerCtx.drawImage(masks[index], 0, 0, size, size);
    finalCtx.drawImage(layerCanvas, 0, 0);
  }

  finalCtx.globalCompositeOperation = 'source-over';
  const frameKey = frameKeyFor(maxFps, options.now);
  blendedTileCache.set(baseKey, { canvas: finalCanvas, animated: maxFps > 0, frameKey, fps: maxFps });
  trimCache(blendedTileCache, MAX_BLEND_CACHE);
  return finalCanvas;
}
