...
import { MAP_PALETTES, MAP_PALETTE_ENTRIES, getPaletteEntry, registerCustomMapAsset } from './mapEditorCatalog';
...
function readImageSize(src: string) {
  return new Promise<{ width: number; height: number }>((resolve, reject) => {
    ...
    image.onerror = () => reject(new Error('PNG inválido.'));
    image.src = src;
  });
}
...
export function startMapEditor() {
  ...
  root.innerHTML = `
    <div class="me-menubar">
      ...
      <button id="me-new">Novo</button><button id="me-save">Salvar</button><button id="me-import">Importar mapa</button><button id="me-export">Exportar</button>
      <button id="me-assets">＋ Asset PNG</button><button class="me-playtest-button" id="me-playtest">▶ Playtest</button>
    ...
    <div class="me-workspace">
      <aside class="me-sidebar"><div class="me-panel-head"><strong>Paletas</strong><span>sprite real</span></div><div class="me-palette-tabs" id="me-palette-tabs"></div><div class="me-search"><input id="me-search" placeholder="Buscar asset..." autocomplete="off"></div><div class="me-palette-list" id="me-palette-list"></div></aside>
      <main class="me-canvas-area" id="me-canvas-area"><canvas class="me-canvas" id="me-canvas"></canvas><div class="me-canvas-overlay"><span class="me-chip" id="me-map-chip"></span><span class="me-chip" id="me-selection-chip"></span><span class="me-chip live">● Preview Live</span></div><div class="me-toast" id="me-toast"></div></main>
      <aside class="me-sidebar right"><div class="me-right-scroll">
        <section class="me-section"><div class="me-panel-head"><strong>Layers</strong><span>visibilidade • lock</span></div><div class="me-section-body me-layers" id="me-layers"></div></section>
    ...
    <input id="me-asset-input" type="file" accept="image/png" hidden>`;
  ...
  const assetInput = root.querySelector<HTMLInputElement>('#me-asset-input')!;
  ...
  const objectAt = (x: number, y: number) => [...mapDoc.objects].reverse().find((value) => {
    const footprint = getPaletteEntry(value.assetId).footprint;
    if (!footprint) return value.x === x && value.y === y;
    const half = Math.floor(footprint.width / 2);
    return x >= value.x - half && x < value.x - half + footprint.width && y >= value.y - footprint.height + 1 && y <= value.y;
  }) ?? null;
  ...
  const schedulePreview = () => {
    ...
    previewTimer = window.setTimeout(publishPreview, 35);
  };
  ...
  const save = () => { mapDoc = saveMapDocument(mapDoc); markSaved(); refreshMapSelect(); refreshChrome(); publishPreview(); showToast('Mapa salvo no navegador.'); };
  ...
  const ensureUnlocked = () => {
    if (!locked[layer]) return true;
    showToast(`Layer ${LAYERS.find((value) => value.id === layer)?.label ?? layer} está bloqueada.`);
    return false;
  };
  ...
  const paintObject = (x: number, y: number) => {
    ...
    const width = entry.footprint?.width ?? Math.max(1, Math.ceil(entry.sprite?.widthTiles ?? 1));
    const height = entry.footprint?.height ?? Math.max(1, Math.ceil(entry.sprite?.heightTiles ?? 1));
    const value: MapObject = { id: uid('object'), kind: entry.objectKind, assetId: entry.id, x, y, width, height, scale: 1, rotation: 0, properties: {} };
  ...
  const eraseAt = (x: number, y: number) => {
    ...
    if (layer === 'objects') {
      ...
      if (hit) mapDoc.objects = mapDoc.objects.filter((value) => value.id !== hit.id);
    } else if (layer === 'zones') {
      const value = zoneAt(x, y); if (value) mapDoc.zones = mapDoc.zones.filter((item) => item.id !== value.id);
    } else if (layer === 'collision') mapDoc.collision = mapDoc.collision.filter((key) => key !== tileKey(x, y));
    else {
      const value = mapDoc.tiles[tileKey(x, y)] ?? {};
      if (layer === 'detail') delete value.detail; else value.ground = 'grass';
      mapDoc.tiles[tileKey(x, y)] = value;
    }
  };
  ...
  const selectEntry = (next: MapPaletteEntry) => { entry = next; layer = next.defaultLayer; if (next.palette !== 'terrain') tool = 'brush'; renderPalette(); renderLayers(); selectTool(tool); refreshInspector(); render(); };
  ...
  const renderPalette = () => {
    ...
    paletteList.innerHTML = values.length ? values.map((value) => {
      const preview = value.sprite?.src
        ? `<span class="me-palette-preview sprite"><img src="${escapeHtml(value.sprite.src)}" alt=""></span>`
        : `<span class="me-palette-preview" style="background:${value.color}">${value.icon}</span>`;
      const source = value.source === 'pixel-crawler' ? ' • Pixel Crawler' : value.source === 'custom' ? ' • Importado' : '';
      return `<button class="me-palette-item ${entry.id === value.id ? 'active' : ''}" data-entry="${value.id}">${preview}<span><strong>${escapeHtml(value.label)}</strong><small>${escapeHtml(value.description)}${source}</small></span></button>`;
  ...
  const renderLayers = () => {
    layersNode.innerHTML = LAYERS.map((value) => `<div class="me-layer ${layer === value.id ? 'active' : ''} ${locked[value.id] ? 'locked' : ''}" data-layer="${value.id}"><button data-visible="${value.id}" title="Mostrar/ocultar">${visible[value.id] ? '◉' : '○'}</button><span>${value.icon} ${value.label}</span><button data-lock="${value.id}" title="Bloquear/desbloquear">${locked[value.id] ? '🔒' : '🔓'}</button><span>${layer === value.id ? '●' : ''}</span></div>`).join('');
  ...
  const deleteSelection = () => {
    ...
    if (!selected || !ensureUnlocked()) return;
  ...
  const refreshInspector = () => {
    const selected = selection;
    if (!selected) {
      inspectorType.textContent = 'asset';
      const sprite = entry.sprite ? `${entry.sprite.nativeWidth}×${entry.sprite.nativeHeight}px` : 'vetorial/fallback';
      const footprint = entry.footprint ? `${entry.footprint.width}×${entry.footprint.height} tiles` : '1×1 tile';
      inspector.innerHTML = `<div class="me-inspector-empty"><strong>${escapeHtml(entry.label)}</strong><br>${escapeHtml(entry.description)}<br><br>Fonte: ${escapeHtml(entry.source ?? 'ascension')}<br>Sprite: ${sprite}<br>Footprint: ${footprint}</div>`;
      return;
    }
    if (selected.kind === 'object') {
      ...
      inspector.innerHTML = `<div class="me-inspector-grid"><div class="me-field wide"><label>Objeto</label><input value="${escapeHtml(catalog.label)}" readonly></div><div class="me-field"><label>X</label><input id="me-ins-x" type="number" min="0" max="${mapDoc.width - 1}" value="${value.x}"></div><div class="me-field"><label>Y</label><input id="me-ins-y" type="number" min="0" max="${mapDoc.height - 1}" value="${value.y}"></div><div class="me-field"><label>Escala</label><input id="me-ins-scale" type="number" min="0.25" max="4" step="0.05" value="${value.scale ?? 1}"></div><div class="me-field"><label>Rotação</label><input id="me-ins-r" type="number" step="15" value="${value.rotation ?? 0}"></div><div class="me-field wide"><label>Fonte / footprint</label><input value="${escapeHtml(catalog.source ?? 'ascension')} • ${catalog.footprint?.width ?? 1}×${catalog.footprint?.height ?? 1}" readonly></div></div><div class="me-inspector-actions"><button class="me-small-btn" id="me-center-selection">Centralizar</button><button class="me-small-btn danger" id="me-delete-selection">Excluir</button></div>`;
      const update = () => {
        beginMutation('Editar objeto');
        value.x = clamp(Number(root.querySelector<HTMLInputElement>('#me-ins-x')!.value) || 0, 0, mapDoc.width - 1);
        value.y = clamp(Number(root.querySelector<HTMLInputElement>('#me-ins-y')!.value) || 0, 0, mapDoc.height - 1);
        value.scale = clamp(Number(root.querySelector<HTMLInputElement>('#me-ins-scale')!.value) || 1, .25, 4);
        value.rotation = Number(root.querySelector<HTMLInputElement>('#me-ins-r')!.value) || 0;
        finishMutation();
      };
      root.querySelector<HTMLInputElement>('#me-ins-x')!.onchange = update;
      root.querySelector<HTMLInputElement>('#me-ins-y')!.onchange = update;
      root.querySelector<HTMLInputElement>('#me-ins-scale')!.onchange = update;
      root.querySelector<HTMLInputElement>('#me-ins-r')!.onchange = update;
    ...
    if (selected.kind === 'zone') {
      ...
      const update = () => {
        beginMutation('Editar zona');
        value.name = root.querySelector<HTMLInputElement>('#me-zone-name')!.value;
        value.x = clamp(Number(root.querySelector<HTMLInputElement>('#me-zone-x')!.value) || 0, 0, mapDoc.width - 1);
        value.y = clamp(Number(root.querySelector<HTMLInputElement>('#me-zone-y')!.value) || 0, 0, mapDoc.height - 1);
        value.width = clamp(Number(root.querySelector<HTMLInputElement>('#me-zone-w')!.value) || 1, 1, mapDoc.width - value.x);
        value.height = clamp(Number(root.querySelector<HTMLInputElement>('#me-zone-h')!.value) || 1, 1, mapDoc.height - value.y);
        finishMutation();
      };
  ...
  const refreshMapSelect = () => {
    mapSelect.innerHTML = listMapDocuments().map((value) => `<option value="${value.id}" ${value.id === mapDoc.id ? 'selected' : ''}>${escapeHtml(value.name)}</option>`).join('');
  };
  const refreshChrome = () => {
    ...
    layerStatus.textContent = `${layerDef.label}${locked[layer] ? ' 🔒' : ''}`;
    ...
    const selected = selection;
    if (!selected) selectionChip.textContent = `${entry.icon} ${entry.label}`;
    else if (selected.kind === 'tile') selectionChip.textContent = `Tile ${selected.x}, ${selected.y}`;
    else selectionChip.textContent = selected.kind === 'object' ? 'Objeto selecionado' : 'Zona selecionada';
  };
  ...
  const renderMinimap = () => {
    ...
    minimapCtx.fillStyle = mapDoc.metadata.background || '#527b45'; minimapCtx.fillRect(0, 0, width, height);
    for (const [key, tile] of Object.entries(mapDoc.tiles)) {
      const point = parseTileKey(key); minimapCtx.fillStyle = getPaletteEntry(tile.ground ?? 'grass').color; minimapCtx.fillRect(point.x * sx, point.y * sy, sx + 1, sy + 1);
    }
  ...
  const render = () => {
    ...
    const ts = mapDoc.tileSize, screenTile = ts * zoom;
    const startX = clamp(Math.floor(cameraX / ts) - 2, 0, mapDoc.width - 1), startY = clamp(Math.floor(cameraY / ts) - 2, 0, mapDoc.height - 1);
    const endX = clamp(Math.ceil((cameraX + view.width / zoom) / ts) + 2, 0, mapDoc.width - 1), endY = clamp(Math.ceil((cameraY + view.height / zoom) / ts) + 2, 0, mapDoc.height - 1);

    for (let y = startY; y <= endY; y++) for (let x = startX; x <= endX; x++) {
      const tile = mapDoc.tiles[tileKey(x, y)] ?? { ground: 'grass' }, sx = (x * ts - cameraX) * zoom, sy = (y * ts - cameraY) * zoom;
      if (visible.ground) drawTerrainAsset(ctx, getPaletteEntry(tile.ground ?? 'grass'), sx, sy, screenTile, 1, render); else { ctx.fillStyle = '#192027'; ctx.fillRect(sx, sy, screenTile + .5, screenTile + .5); }
      if (visible.detail && tile.detail) drawTerrainAsset(ctx, getPaletteEntry(tile.detail), sx, sy, screenTile, .72, render);
    }
    ...
    if (visible.collision) {
      ...
      for (const key of mapDoc.collision) { const point = parseTileKey(key); if (point.x < startX || point.x > endX || point.y < startY || point.y > endY) continue; const sx = (point.x * ts - cameraX) * zoom, sy = (point.y * ts - cameraY) * zoom; ctx.fillRect(sx, sy, screenTile, screenTile); ctx.strokeRect(sx + 1, sy + 1, screenTile - 2, screenTile - 2); }
    }
    if (visible.objects) {
      const objects = [...mapDoc.objects].sort((a, b) => a.y - b.y);
      for (const value of objects) {
        drawObjectAsset(ctx, getPaletteEntry(value.assetId), { x: ((value.x + .5) * ts - cameraX) * zoom, y: ((value.y + 1) * ts - cameraY) * zoom, tilePixels: screenTile, scale: value.scale ?? 1, selected: selection?.kind === 'object' && selection.id === value.id, onReady: render });
      }
    }
    ...
    const selected = selection;
    if (selected?.kind === 'tile') { const sx = (selected.x * ts - cameraX) * zoom, sy = (selected.y * ts - cameraY) * zoom; ctx.strokeStyle = '#dff4ff'; ctx.lineWidth = 2; ctx.strokeRect(sx + 1, sy + 1, screenTile - 2, screenTile - 2); }

    const hover = hoverTile;
    if (hover && validTile(hover.x, hover.y) && tool === 'brush' && !locked[layer]) {
      const sx = (hover.x * ts - cameraX) * zoom, sy = (hover.y * ts - cameraY) * zoom;
      if (entry.palette === 'terrain') drawTerrainAsset(ctx, entry, sx, sy, screenTile, .45, render);
      else if (entry.palette !== 'zone' && entry.objectKind) drawObjectAsset(ctx, entry, { x: ((hover.x + .5) * ts - cameraX) * zoom, y: ((hover.y + 1) * ts - cameraY) * zoom, tilePixels: screenTile, alpha: .45, onReady: render });
    }
    renderMinimap();
  };

  const openNewMapModal = () => {
    const backdrop = document.createElement('div'); backdrop.className = 'me-modal-backdrop';
    backdrop.innerHTML = `<div class="me-modal"><header><h3>Novo mapa</h3><p>O tamanho é definido em tiles lógicos.</p></header><form id="me-new-form"><div class="me-field wide"><label>Nome</label><input id="me-new-name" value="Novo Mapa" maxlength="60" required></div><div class="me-field"><label>Largura</label><input id="me-new-width" type="number" min="8" max="512" value="69"></div><div class="me-field"><label>Altura</label><input id="me-new-height" type="number" min="8" max="512" value="50"></div><footer><button type="button" id="me-new-cancel">Cancelar</button><button class="primary" type="submit">Criar mapa</button></footer></form></div>`;
    document.body.appendChild(backdrop); backdrop.querySelector<HTMLButtonElement>('#me-new-cancel')!.onclick = () => backdrop.remove();
    backdrop.querySelector<HTMLFormElement>('#me-new-form')!.onsubmit = (event) => { event.preventDefault(); const name = backdrop.querySelector<HTMLInputElement>('#me-new-name')!.value.trim() || 'Novo Mapa'; const width = Number(backdrop.querySelector<HTMLInputElement>('#me-new-width')!.value) || 69; const height = Number(backdrop.querySelector<HTMLInputElement>('#me-new-height')!.value) || 50; mapDoc = saveMapDocument(createBlankMap(name, width, height)); undoStack.length = 0; redoStack.length = 0; selection = null; markSaved(); backdrop.remove(); fitMap(); refreshAll(); publishPreview(); showToast(`Mapa ${name} criado.`); };
  };

  const openMapSettings = () => {
    const backdrop = document.createElement('div'); backdrop.className = 'me-modal-backdrop';
    backdrop.innerHTML = `<div class="me-modal"><header><h3>Configurações do mapa</h3><p>Metadados usados pelo jogo e futuros editores.</p></header><form id="me-settings-form"><div class="me-field wide"><label>Nome</label><input id="me-set-name" maxlength="60" value="${escapeHtml(mapDoc.name)}"></div><div class="me-field"><label>Level recomendado</label><input id="me-set-level" maxlength="30" value="${escapeHtml(mapDoc.metadata.recommendedLevel ?? '')}"></div><div class="me-field"><label>Tile size</label><input value="${mapDoc.tileSize}px" readonly></div><div class="me-field wide"><label>Notas</label><textarea id="me-set-notes">${escapeHtml(mapDoc.metadata.notes ?? '')}</textarea></div><footer><button type="button" id="me-settings-cancel">Cancelar</button><button class="primary" type="submit">Salvar</button></footer></form></div>`;
    document.body.appendChild(backdrop); backdrop.querySelector<HTMLButtonElement>('#me-settings-cancel')!.onclick = () => backdrop.remove();
    backdrop.querySelector<HTMLFormElement>('#me-settings-form')!.onsubmit = (event) => { event.preventDefault(); beginMutation('Editar mapa'); mapDoc.name = backdrop.querySelector<HTMLInputElement>('#me-set-name')!.value.trim() || mapDoc.name; mapDoc.metadata.recommendedLevel = backdrop.querySelector<HTMLInputElement>('#me-set-level')!.value; mapDoc.metadata.notes = backdrop.querySelector<HTMLTextAreaElement>('#me-set-notes')!.value; finishMutation(); backdrop.remove(); refreshMapSelect(); };
  };

  const openAssetImportModal = async (file: File) => {
    if (file.size > 1_500_000) { showToast('PNG muito grande para o armazenamento local. Limite: 1,5 MB.'); return; }
    try {
      const dataUrl = await readFileAsDataUrl(file);
      const size = await readImageSize(dataUrl);
      const defaultW = Math.max(.25, Math.round(size.width / 32 * 4) / 4);
      const defaultH = Math.max(.25, Math.round(size.height / 32 * 4) / 4);
      const backdrop = document.createElement('div'); backdrop.className = 'me-modal-backdrop';
      backdrop.innerHTML = `<div class="me-modal"><header><h3>Importar asset PNG</h3><p>${size.width}×${size.height}px • salvo localmente neste navegador.</p></header><form id="me-asset-form"><div class="me-asset-import-preview"><img src="${escapeHtml(dataUrl)}" alt="preview"></div><div class="me-field wide"><label>Nome</label><input id="me-asset-name" maxlength="60" value="${escapeHtml(file.name.replace(/\.png$/i, ''))}"></div><div class="me-field"><label>Categoria</label><select id="me-asset-category"><option value="doodad">Cenário</option><option value="npc">NPC</option><option value="monster">Monstro</option><option value="resource">Recurso</option><option value="raw">Raw</option></select></div><div class="me-field"><label>Tipo</label><input value="PNG / pixel" readonly></div><div class="me-field"><label>Largura em tiles</label><input id="me-asset-w" type="number" min="0.25" max="32" step="0.25" value="${defaultW}"></div><div class="me-field"><label>Altura em tiles</label><input id="me-asset-h" type="number" min="0.25" max="32" step="0.25" value="${defaultH}"></div><footer><button type="button" id="me-asset-cancel">Cancelar</button><button class="primary" type="submit">Adicionar à paleta</button></footer></form></div>`;
      document.body.appendChild(backdrop);
      backdrop.querySelector<HTMLButtonElement>('#me-asset-cancel')!.onclick = () => backdrop.remove();
      backdrop.querySelector<HTMLFormElement>('#me-asset-form')!.onsubmit = (event) => {
        event.preventDefault();
        const kind = backdrop.querySelector<HTMLSelectElement>('#me-asset-category')!.value as MapObject['kind'];
        const targetPalette = kind === 'doodad' ? 'doodad' : kind as MapPaletteId;
        const imported = registerCustomMapAsset({ label: backdrop.querySelector<HTMLInputElement>('#me-asset-name')!.value.trim() || 'Asset importado', dataUrl, palette: targetPalette, objectKind: kind, widthTiles: clamp(Number(backdrop.querySelector<HTMLInputElement>('#me-asset-w')!.value) || defaultW, .25, 32), heightTiles: clamp(Number(backdrop.querySelector<HTMLInputElement>('#me-asset-h')!.value) || defaultH, .25, 32) });
        backdrop.remove(); palette = imported.palette; selectEntry(imported); preloadMapAssets([imported], render); showToast('Asset adicionado à paleta Raw/Cenário.');
      };
    } catch (error) { showToast(error instanceof Error ? error.message : 'Falha ao importar PNG.'); }
  };
  ...
  root.querySelector<HTMLButtonElement>('#me-assets')!.onclick = () => assetInput.click();
  root.querySelector<HTMLButtonElement>('#me-playtest')!.onclick = () => { publishPreview(); window.open(`${location.pathname}?playtest=map&id=${encodeURIComponent(mapDoc.id)}`, '_blank'); };
  ...
  root.querySelector<HTMLButtonElement>('#me-back-game')!.onclick = () => { if (dirty && !window.confirm('Há alterações não salvas. Voltar ao jogo principal mesmo assim?')) return; location.href = location.pathname; };
  ...
  fileInput.onchange = async () => {
    ...
    try { mapDoc = importMapDocument(await file.text()); selection = null; undoStack.length = 0; redoStack.length = 0; markSaved(); fitMap(); refreshAll(); publishPreview(); showToast(`Mapa ${mapDoc.name} importado.`); } catch (error) { showToast(error instanceof Error ? error.message : 'Falha ao importar mapa.'); }
    fileInput.value = '';
  };
  assetInput.onchange = () => { const file = assetInput.files?.[0]; if (file) void openAssetImportModal(file); assetInput.value = ''; };
  mapSelect.onchange = () => {
    ...
    mapDoc = next; selection = null; undoStack.length = 0; redoStack.length = 0; markSaved(); fitMap(); refreshAll(); publishPreview();
  };
  ...
  canvas.addEventListener('pointerdown', (event) => {
    ...
    if (tool === 'select') {
      ...
      const selected = selection;
      if ((selected?.kind === 'object' && !locked.objects) || (selected?.kind === 'zone' && !locked.zones)) { beginMutation('Mover seleção'); dragMode = 'move'; }
      return;
    }
  ...
  canvas.addEventListener('pointermove', (event) => {
    ...
    if (dragMode === 'move' && validTile(point.x, point.y)) {
      const selected = selection;
      if (selected?.kind === 'object') { const value = mapDoc.objects.find((item) => item.id === selected.id); if (value) { value.x = point.x; value.y = point.y; actionOpen = true; } }
      else if (selected?.kind === 'zone') { const value = mapDoc.zones.find((item) => item.id === selected.id); if (value) { value.x = clamp(point.x, 0, mapDoc.width - value.width); value.y = clamp(point.y, 0, mapDoc.height - value.height); actionOpen = true; } }
      refreshInspector(); render(); return;
    }
    if (dragMode === 'paint') useToolAt(point.x, point.y, false); else render();
  });
  canvas.addEventListener('pointerleave', () => { hoverTile = null; if (dragMode === 'none') render(); });
  ...
  canvas.addEventListener('wheel', (event) => {
    event.preventDefault(); const rect = canvas.getBoundingClientRect(), sx = event.clientX - rect.left, sy = event.clientY - rect.top, beforeX = sx / zoom + cameraX, beforeY = sy / zoom + cameraY;
    zoom = clamp(zoom * (event.deltaY < 0 ? 1.12 : .89), .15, 3.2); cameraX = beforeX - sx / zoom; cameraY = beforeY - sy / zoom; clampCamera(); refreshChrome(); render();
  }, { passive: false });
  ...
  const resizeCanvas = () => {
    const rect = canvasArea.getBoundingClientRect(), dpr = Math.min(2, devicePixelRatio || 1);
  ...
  window.addEventListener('pagehide', () => { previewPublisher.close(); observer.disconnect(); }, { once: true });

  preloadMapAssets(MAP_PALETTE_ENTRIES, render);
  selectTool(tool); refreshAll(); resizeCanvas(); publishPreview();
  showToast('Preview real ativo. ▶ Playtest testa o mapa sem publicar no jogo principal.');

  return { get document() { return mapDoc; }, save, destroy() { previewPublisher.close(); observer.disconnect(); root.remove(); } };
}
