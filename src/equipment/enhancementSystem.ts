...
function copy(meta: ItemEnhancementData) { return { refine: meta.refine, gems: [...meta.gems] }; }
...
export function getEquipmentEnhancement(progress: CharacterProgress, slot: EquipmentSlot) {
  ...
  const current = normalizeEnhancement(state.equipmentEnhancements?.[slot]);
  state.equipmentEnhancements ??= {};
  if (state.equipment[slot]) state.equipmentEnhancements[slot] = current;
  return current;
}
...
export function socketLimit(progress: CharacterProgress, slot: EquipmentSlot) {
  const kind = socketKindForSlot(progress, slot);
  return kind ? SOCKET_RULES[kind].maxSockets : 0;
}

export function socketView(progress: CharacterProgress, slot: EquipmentSlot) {
  const limit = socketLimit(progress, slot), meta = getEquipmentEnhancement(progress, slot);
  return Array.from({ length: limit }, (_, index) => meta.gems[index] ?? null);
}

export function nextRefineRule(progress: CharacterProgress, slot: EquipmentSlot) {
  const state = ensureInventoryState(progress);
  if (!state.equipment[slot]) return null;
  const current = getEquipmentEnhancement(progress, slot).refine;
  return REFINEMENT_CONFIG.levels.find((rule) => rule.targetLevel === current + 1) ?? null;
}
...
export function refineEquipment(progress: CharacterProgress, slot: EquipmentSlot, roll = Math.random()): RefineResult {
  ...
  const rule = nextRefineRule(progress, slot);
  ...
  const afterMeta = copy(beforeMeta);
  ...
  state.equipmentEnhancements ??= {}; state.equipmentEnhancements[slot] = afterMeta;
  return { ok: true, success, before: beforeMeta.refine, after: afterMeta.refine, chance: rule.successChance };
}

export function socketGem(progress: CharacterProgress, slot: EquipmentSlot, gemId: string) {
  const state = ensureInventoryState(progress), itemId = state.equipment[slot];
  if (!itemId) return { ok: false, reason: 'Não há equipamento neste slot.' };
  const kind = socketKindForSlot(progress, slot);
  if (!kind) return { ok: false, reason: 'Este tipo de item não aceita pedras de soquete.' };
  ...
  const beforeMeta = copy(getEquipmentEnhancement(progress, slot));
  const gems = Array.from({ length: SOCKET_RULES[kind].maxSockets }, (_, index) => beforeMeta.gems[index] ?? null);
  const empty = gems.findIndex((value) => !value);
  ...
  const afterMeta = { refine: beforeMeta.refine, gems: [...gems] };
  afterMeta.gems[empty] = gemId;
  applyEquipmentStatDelta(progress, slot, beforeMeta, afterMeta);
  state.equipmentEnhancements ??= {}; state.equipmentEnhancements[slot] = afterMeta;
  return { ok: true, socket: empty, gem };
}

export function removeSocketGem(progress: CharacterProgress, slot: EquipmentSlot, socketIndex: number) {
  const state = ensureInventoryState(progress), beforeMeta = copy(getEquipmentEnhancement(progress, slot));
  const gemId = beforeMeta.gems[socketIndex];
  if (!gemId) return { ok: false, reason: 'Este soquete está vazio.' };
  const returned = addItem(progress, gemId, 1);
  if (returned.remaining > 0) return { ok: false, reason: 'Inventário cheio. Libere espaço antes de remover a pedra.' };
