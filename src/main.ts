...
async function startGame() {
  try {
    ...
    function addTree(x: number, y: number, scale = 1) {
      ...
      tree.addChild(
        new Graphics().ellipse(0, 26, 32 * scale, 13 * scale).fill({ color: 0, alpha: 0.18 }),
    ...
    treePositions.forEach(([x, y], i) => addTree(x, y, i % 3 === 0 ? 1.15 : 1));

    const npc = new Container();
    const npcMark = new Text({ text: '!', style: { fill: 0xffdd57, fontSize: 32, fontWeight: 'bold', stroke: { color: 0, width: 5 } } });
    npcMark.anchor.set(0.5); npcMark.y = -82;
    const npcName = new Text({ text: 'Elandra', style: { fill: 0xffffff, fontSize: 14, fontWeight: 'bold', stroke: { color: 0, width: 4 } } });
    npcName.anchor.set(0.5); npcName.y = -62;
    npc.addChild(
      new Graphics().ellipse(0, 22, 22, 9).fill({ color: 0, alpha: 0.22 }),
      new Graphics().roundRect(-18, -28, 36, 50, 9).fill(0x4f78b8).stroke({ width: 3, color: 0xbfd8ff }),
      new Graphics().circle(0, -39, 14).fill(0xe4b991),
      npcMark,
      npcName,
    );
    ...
    const enemyName = new Text({ text: 'Lobo Sombrio', style: { fill: 0xffd0ca, fontSize: 13, fontWeight: 'bold', stroke: { color: 0, width: 4 } } });
    enemyName.anchor.set(0.5); enemyName.y = -50;
    const enemyHpBar = new Graphics().roundRect(-30, -38, 60, 5, 2).fill(0xdb5b52);
    enemy.addChild(
      new Graphics().ellipse(0, 19, 23, 9).fill({ color: 0, alpha: 0.22 }),
      new Graphics().circle(0, -5, 27).fill(0x7a3b35).stroke({ width: 3, color: 0xc66a58 }),
      new Graphics().circle(-9, -11, 4).fill(0xffe06b),
      new Graphics().circle(9, -11, 4).fill(0xffe06b),
      enemyName,
      new Graphics().roundRect(-31, -39, 62, 7, 3).fill(0x301718),
      enemyHpBar,
    );
    ...
    loot.addChild(
      new Graphics().circle(0, 0, 16).fill({ color: 0xffd35a, alpha: 0.25 }),
      new Graphics().circle(0, 0, 7).fill(0xffd35a),
    );
    loot.visible = false;
    world.addChild(loot);
    ...
    function interact() {
      if (distance(player.x, player.y, npc.x, npc.y) < 115) {
        if (!questAccepted) {
          ...
          showDialog('Elandra: Excelente! Recompensa: 25 moedas.');
        } else {
    ...
    function attack() {
      if (!enemyAlive || attackCooldown > 0 || hero.isAttacking) return;
    ...
    function resetStick() { stickX = 0; stickY = 0; knob.style.transform = 'translate(0, 0)'; }
    stick.addEventListener('pointerdown', (event) => { stick.setPointerCapture(event.pointerId); updateStick(event.clientX, event.clientY); });
    stick.addEventListener('pointermove', (event) => { if (stick.hasPointerCapture(event.pointerId)) updateStick(event.clientX, event.clientY); });
    ...
    app.ticker.add((ticker) => {
      ...
      const len = Math.hypot(dx, dy);
      const moving = len > 0 && !hero.isAttacking;
      if (moving) {
      ...
      hero.update(moving, dt);

      if (enemyAlive) {
        const d = distance(enemy.x, enemy.y, player.x, player.y);
        if (d < 360 && d > 68) {
          enemy.x += (player.x - enemy.x) / d * 2.05 * dt;
          enemy.y += (player.y - enemy.y) / d * 2.05 * dt;
        }
