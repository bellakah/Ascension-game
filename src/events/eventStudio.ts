...
const ACTIONS: Record<EventActionType, string> = { mission: 'Ativar missão', 'spawn-group': 'Spawn Group', portal: 'Portal', marker: 'Marker', shop: 'Loja', 'drop-table': 'Drop Table', buff: 'Buff global', 'unlock-class': 'Desbloquear classe', 'change-class': 'Trocar classe', 'learn-skill': 'Ensinar Skill' };
...
export function createEventStudio(host: HTMLElement) {
  ...
  const classOptions = (selected = '') => `<option value="">Selecione uma classe...</option>${listClassDefinitions({ publishedOnly: true }).map((entry) => option(entry.id, `#${entry.numericId} · ${entry.name}`, selected)).join('')}`;
  const skillOptions = (selected = '') => `<option value="">Selecione uma Skill...</option>${listPublishedSkills().map((entry) => option(entry.id, `#${entry.numericId} · ${entry.name}`, selected)).join('')}`;
  ...
  function actionTarget(action: EventStudioRecord['actions'][number], index: number) {
    ...
    if (action.type === 'unlock-class' || action.type === 'change-class') return `<select data-action-target data-ai="${index}">${classOptions(action.targetId)}</select>`;
    if (action.type === 'learn-skill') return `<select data-action-target data-ai="${index}">${skillOptions(action.targetId)}</select>`;
    return `<input data-action-target data-ai="${index}" value="${esc(action.targetId)}" placeholder="ID do ${esc(ACTIONS[action.type])}">`;
  }

  function actions() {
    return `<section class="mission-section"><div class="mission-section-head"><strong>Ações do evento</strong><button class="mission-btn" data-add-action>+ Ação</button></div><div class="mission-section-body">${current.actions.length ? current.actions.map((action,index) => `<div class="mission-stage" style="margin-bottom:8px"><div class="mission-stage-head"><input type="checkbox" data-action-enabled data-ai="${index}" ${action.enabled ? 'checked' : ''} title="Ativa"><select data-action-type data-ai="${index}" style="flex:0 0 180px">${(Object.keys(ACTIONS) as EventActionType[]).map((type) => option(type,ACTIONS[type],action.type)).join('')}</select><input data-action-label data-ai="${index}" value="${esc(action.label)}" placeholder="Nome interno da ação">${actionTarget(action,index)}<button class="mission-icon-btn" data-remove-action data-ai="${index}">×</button></div></div>`).join('') : '<div class="mission-empty-note">Nenhuma ação. Adicione uma missão ou outro conteúdo controlado pelo evento.</div>'}<p style="color:#748a91"><b>Missões e ações de classe</b> possuem adaptador de runtime. Ações de classe são aplicadas uma vez por personagem enquanto o evento estiver ativo.</p></div></section>`;
  }

  function test() {
    const active = isEventRecordActive(current); const missionActions = current.actions.filter((action) => action.enabled && action.type === 'mission'); const classActions = current.actions.filter((action) => action.enabled && ['unlock-class','change-class','learn-skill'].includes(action.type));
    return `<section class="mission-section"><div class="mission-section-head"><strong>Simulador do evento</strong></div><div class="mission-section-body"><div class="mission-sim-card"><header><strong>${active ? 'Evento ativo neste instante' : 'Evento inativo neste instante'}</strong><span>${new Date().toLocaleString('pt-BR')}</span></header><p>Status: ${esc(current.status)} · Agenda: ${esc(current.schedule.mode)}</p></div><strong>Missões controladas</strong>${missionActions.length ? missionActions.map((action) => { const mission = listMissionStudioRecords().find((entry) => entry.key === action.targetId); return `<div class="mission-sim-card"><header><strong>${esc(mission?.title || action.targetId)}</strong><span>${active ? 'HABILITADA' : 'BLOQUEADA'}</span></header></div>`; }).join('') : '<div class="mission-empty-note">Nenhuma missão vinculada.</div>'}<strong>Ações de classe</strong>${classActions.length ? classActions.map((action) => `<div class="mission-sim-card"><header><strong>${esc(ACTIONS[action.type])}</strong><span>${esc(action.targetId)}</span></header></div>`).join('') : '<div class="mission-empty-note">Nenhuma ação de classe vinculada.</div>'}</div></section>`;
  }
