...
type Animation = 'idle' | 'walk' | 'slash' | 'backslash' | 'halfslash' | 'spellcast' | 'emote';
...
export class LpcCharacter {
  ...
  static async create(config: CharacterConfig = DEFAULT_CHARACTER, equipment: CharacterEquipmentVisuals = {}) {
    ...
    await character.load(); character.render(); return character;
  }
  private constructor(config: CharacterConfig, equipment: CharacterEquipmentVisuals) {
    this.config = config; this.equipment = normalizeEquipment(equipment); this.view.sortableChildren = true;
    ...
    for (const [name,zIndex] of order) { const sprite = new Sprite(); sprite.anchor.set(.5,1); sprite.zIndex = zIndex; this.layers.set(name,sprite); this.view.addChild(sprite); }
  }
  ...
  private weaponPath(animation: Animation, side: 'bg'|'fg') {
    if (animation === 'backslash' || animation === 'halfslash') return null;
    if (this.isStaff()) {
    ...
    const material = this.swordMaterial(); if (!material) return null;
  ...
  private async tryLoad(key: string, assetUrl: string) {
    try { const texture = await Assets.load<Texture>(assetUrl); texture.source.scaleMode = 'nearest'; this.sheets.set(key, texture); }
    catch (error) { console.warn(`[LPC] asset opcional ignorado: ${key}`, assetUrl, error); }
  }
  private animations(): Animation[] { return ['idle','walk','slash','backslash','halfslash','spellcast','emote']; }
  ...
  private async loadEquipmentSheets() {
    ...
    for (const animation of this.animations()) {
      ...
      if (armor) jobs.push(this.tryLoad(`mail:${animation}`, armor)); if (hood) jobs.push(this.tryLoad(`hood:${animation}`, hood));
      for (const side of ['bg','fg'] as const) { const weapon = this.weaponPath(animation, side); if (!weapon) continue; jobs.push(this.tryLoad(`${side === 'bg' ? 'weaponBg' : 'weaponFg'}:${animation}`, weapon)); }
    }
    await Promise.all(jobs);
  }
  private async load() { await Promise.all([this.loadBaseSheets(), this.loadEquipmentSheets()]); if (!this.sheets.has('body:idle')) throw new Error('Não foi possível carregar o corpo LPC básico.'); }
  async setEquipment(equipment: CharacterEquipmentVisuals) { const next = normalizeEquipment(equipment); if (JSON.stringify(next) === JSON.stringify(this.equipment)) return; this.equipment = next; await this.loadEquipmentSheets(); this.render(); }
  setFacing(facing: Facing) { this.facing = facing; this.render(); }
  private startAction(animation: Exclude<Animation,'idle'|'walk'>) { if (this.isAttacking) return false; this.isAttacking = true; this.animation = animation; this.frame = 0; this.clock = 0; this.render(); return true; }
  attack() { return this.startAction('slash'); }
  cast() { return this.startAction('spellcast'); }
  emote() { return this.startAction('emote'); }
  gather(action: LpcGatheringAction) {
    const animation: Animation = action === 'chop' ? 'backslash' : action === 'mine' || action === 'dig' ? 'halfslash' : 'emote';
    return this.startAction(animation as Exclude<Animation,'idle'|'walk'>);
  }
  update(moving: boolean, dt: number) {
    if (this.isAttacking) {
      ...
      if (this.clock >= actionSpeed) {
        this.clock = 0; this.frame += 1;
        if (this.frame >= this.frameCount('body', this.animation)) { this.isAttacking = false; this.animation = moving ? 'walk' : 'idle'; this.frame = 0; }
        this.render();
      }
      return;
    }
    ...
    this.clock += dt; const speed = moving ? 6 : 16;
    if (this.clock >= speed) { this.clock = 0; this.frame = (this.frame + 1) % this.frameCount('body', this.animation); }
    this.render();
  }
  private frameCount(layer: LayerName, animation: Animation) { const sheet = this.sheets.get(`${layer}:${animation}`) ?? this.sheets.get(`${layer}:idle`); if (!sheet) return 1; const size = sheet.height / 4; return Math.max(1, Math.floor(sheet.width / size)); }
  private crop(sheet: Texture, frame: number) { const size = sheet.height / 4; const count = Math.max(1, Math.floor(sheet.width / size)); const safeFrame = Math.min(frame, count - 1); return new Texture({ source: sheet.source, frame: new Rectangle(safeFrame * size, row[this.facing] * size, size, size) }); }
  private tintFor(name: LayerName) {
    if (name === 'body' || name === 'head') return this.config.skinColor; if (name === 'hair') return this.config.hairColor;
    if (name === 'pants') return this.equipment.legs === 'ranger_legs' ? 0x88906a : 0x806653; if (name === 'boots') return this.equipment.boots === 'forest_boots' ? 0x68825d : 0x9a7658; if (name === 'hood') return 0x74594d; return 0xffffff;
  }
  private shouldShow(name: LayerName) {
    ...
    if (name === 'mail') return Boolean(this.equipment.armor); if (name === 'hood') return Boolean(this.equipment.head && (this.sheets.has(`hood:${this.animation}`) || this.sheets.has('hood:idle'))); if (name === 'boots') return Boolean(this.equipment.boots); if (name === 'weaponBg' || name === 'weaponFg') return Boolean(this.equipment.weapon); return true;
  }
  private render() {
    for (const [name,sprite] of this.layers) {
      ...
      sprite.visible = true; const actionAnchor = (this.animation === 'slash' || this.animation === 'backslash' || this.animation === 'halfslash') && Boolean(exact);
      sprite.anchor.set(.5, actionAnchor ? 2 / 3 : 1); sprite.scale.set(this.scale * bodyWidth[this.config.bodyType], this.scale); sprite.tint = this.tintFor(name); sprite.texture = this.crop(sheet, exact ? this.frame : 0); sprite.texture.source.scaleMode = 'nearest';
    }
  }
}
