...
export function createCraftingUi(progress: CharacterProgress, options: CraftingUiOptions) {
  ...
  overlay.innerHTML = `
    <section class="crafting-window" role="dialog" aria-modal="true" aria-label="Fabricação">
      <header class="crafting-header">
        ...
        <button class="crafting-close" aria-label="Fechar">×</button>
      </header>
  ...
  function renderList() {
    ...
    list.innerHTML = '';
    if (!recipes.length) {
      list.innerHTML = '<div style="padding:16px;color:#93a198;font-size:12px">Nenhuma receita nesta categoria.</div>';
      return;
    }
    ...
    for (const recipe of recipes) {
      ...
      const possible = maxCraftable(progress, recipe);
      const button = document.createElement('button');
      button.className = `recipe-card${selectedId === recipe.id ? ' selected' : ''}${possible <= 0 ? ' disabled' : ''}`;
      button.innerHTML = `
        <span class="recipe-icon">${output?.icon ?? recipe.icon}</span>
        <span><strong>${recipe.name}</strong><small>${recipe.category === 'refining' ? 'Refino' : CRAFTING_CATEGORY_LABELS[recipe.category]}</small></span>
        <span class="recipe-ready">${possible > 0 ? `×${possible}` : 'FALTAM'}</span>`;
      button.addEventListener('click', () => { selectedId = recipe.id; amount = 1; refresh(); });
  ...
  function renderDetail() {
    ...
    if (!recipe) { detail.innerHTML = ''; return; }
    selectedId = recipe.id;
    const max = maxCraftable(progress, recipe);
    amount = Math.max(1, Math.min(amount, Math.max(1, max)));
    const output = getItem(recipe.output.itemId);
    const ingredients = recipe.inputs.map((input) => {
      const item = getItem(input.itemId);
      const owned = itemQuantity(progress, input.itemId);
      const needed = input.quantity * amount;
    ...
    detail.innerHTML = `
      <h3>${output?.icon ?? recipe.icon} ${recipe.name}</h3>
      <p>${recipe.description}</p>
      <strong style="font-size:11px;color:#d8bd6e">MATERIAIS</strong>
      <div class="ingredient-list">${ingredients}</div>
      <div class="craft-output">Resultado: <strong>${recipe.output.quantity * amount}x ${output?.name ?? recipe.output.itemId}</strong>${recipe.requiredLevel ? ` · Nível ${recipe.requiredLevel}` : ''}</div>
      <div class="craft-actions">
        <button id="craft-minus">−</button><span class="craft-qty">${amount}</span><button id="craft-plus">+</button>
        <button class="craft-confirm" id="craft-confirm" ${max <= 0 ? 'disabled' : ''}>⚒ Fabricar</button>
      </div>`;
    ...
    detail.querySelector<HTMLButtonElement>('#craft-confirm')?.addEventListener('click', () => {
      ...
      options.notify(`${made}x ${output?.name ?? recipe.name} fabricado com sucesso.`);
      options.onCrafted?.(recipe, amount);
      options.onChanged();
      amount = 1;
      refresh();
    });
  ...
  function open(type: CraftingStationType) {
    station = type; category = 'all'; selectedId = null; amount = 1;
    overlay.classList.remove('crafting-hidden'); refresh();
  }
