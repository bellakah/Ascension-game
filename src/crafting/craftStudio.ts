...
import {
  craftRecipeDisplay,
  craftStationDisplay,
  createCraftRecipeRecord,
  createCraftStationTypeRecord,
  deleteCraftRecipeRecord,
  deleteCraftStationTypeRecord,
  duplicateCraftRecipeRecord,
  duplicateCraftStationTypeRecord,
  listCraftRecipeRecords,
  listCraftStationTypeRecords,
  saveCraftRecipeRecord,
  saveCraftStationTypeRecord,
...
export function createCraftStudio(host: HTMLElement) {
  ...
  let recipeRecords = listCraftRecipeRecords();
  let stationRecords = listCraftStationTypeRecords();
  let recipe = clone(recipeRecords[0] ?? createCraftRecipeRecord());
  let station = clone(stationRecords[0] ?? createCraftStationTypeRecord());
  let mode: CatalogMode = 'recipes';
  let recipeTab: RecipeTab = 'general';
  let query = '';
  let toastTimer = 0;
  ...
  function catalogHtml() {
    ...
    return `<aside class="craft-catalog"><div class="craft-catalog-head"><strong>CRAFT STUDIO</strong><div class="craft-mode-switch"><button id="craft-mode-recipes" class="${mode === 'recipes' ? 'active' : ''}">Receitas</button><button id="craft-mode-stations" class="${mode === 'stations' ? 'active' : ''}">Estações</button></div><div class="craft-catalog-tools"><input id="craft-search" value="${esc(query)}" placeholder="Buscar..."><button id="craft-new">＋</button></div></div><div class="craft-catalog-list">${filtered.length ? filtered.map((entry) => `<button class="craft-card ${(mode === 'recipes' ? recipe.numericId : station.numericId) === entry.numericId ? 'active' : ''}" data-record-id="${entry.numericId}"><span class="craft-card-icon">${esc(entry.icon)}</span><span><strong>#${entry.numericId} · ${esc(entry.name)}</strong><small>${esc(mode === 'recipes' ? (entry as CraftRecipeStudioRecord).category : entry.key)}</small></span><span class="craft-status">${STATUS_LABEL[entry.status]}</span></button>`).join('') : '<div class="craft-empty">Nenhum registro encontrado.</div>'}</div></aside>`;
  }

  function recipeGeneral() {
    return `<section class="craft-section"><div class="craft-section-head"><strong>Identidade da receita</strong><span>Recipe #${recipe.numericId}</span></div><div class="craft-section-body"><div class="craft-grid">
      <div class="craft-field"><label>Nome</label><input data-rfield="name" value="${esc(recipe.name)}"></div>
      <div class="craft-field"><label>Chave interna</label><input data-rfield="key" value="${esc(recipe.key)}" ${recipe.source === 'legacy' ? 'readonly' : ''}></div>
      <div class="craft-field"><label>Categoria</label><input data-rfield="category" value="${esc(recipe.category)}" placeholder="weapon, consumable..."></div>
      <div class="craft-field"><label>Ícone</label><input data-rfield="icon" value="${esc(recipe.icon)}"></div>
      <div class="craft-field"><label>Estação</label><select id="recipe-station">${stationOptions(recipe.stationTypeId)}</select></div>
      <div class="craft-field"><label>Status</label><select id="recipe-status">${(['draft','published','disabled'] as CraftStudioStatus[]).map((value) => `<option value="${value}" ${recipe.status === value ? 'selected' : ''}>${STATUS_LABEL[value]}</option>`).join('')}</select></div>
      <div class="craft-field"><label>Ordem</label><input id="recipe-sort" type="number" value="${recipe.sortOrder}"></div>
      <div class="craft-field full"><label>Descrição</label><textarea data-rfield="description">${esc(recipe.description)}</textarea></div>
      <div class="craft-field full"><label>Tags</label><input id="recipe-tags" value="${esc(recipe.tags.join(', '))}" placeholder="ferreiro, espada, nível-10"></div>
    </div></div></section>`;
  }

  function recipeIngredients() {
    return `<section class="craft-section"><div class="craft-section-head"><strong>Ingredientes</strong><span>Ferramentas podem ser exigidas sem serem consumidas</span><div style="flex:1"></div><button id="ingredient-add" class="craft-btn">＋ Ingrediente</button></div><div class="craft-section-body"><div class="craft-row-list">${recipe.ingredients.length ? recipe.ingredients.map((input,index) => `<div class="craft-row"><select data-ing-item="${index}">${itemOptions(input.itemId)}</select><input data-ing-qty="${index}" type="number" min="1" value="${input.quantity}" title="Quantidade"><label class="craft-check optional-hide"><input data-ing-consume="${index}" type="checkbox" ${input.consume ? 'checked' : ''}> Consumir</label><button data-ing-remove="${index}">×</button></div>`).join('') : '<div class="craft-empty">Nenhum ingrediente. Adicione materiais, ferramentas ou catalisadores.</div>'}</div></div></section>`;
  }

  function recipeOutputs() {
    return `<section class="craft-section"><div class="craft-section-head"><strong>Resultados</strong><span>Resultado principal e subprodutos</span><div style="flex:1"></div><button id="output-add" class="craft-btn">＋ Resultado</button></div><div class="craft-section-body"><div class="craft-row-list">${recipe.outputs.length ? recipe.outputs.map((output,index) => `<div class="craft-row output"><select data-out-item="${index}">${itemOptions(output.itemId)}</select><input data-out-qty="${index}" type="number" min="1" value="${output.quantity}" title="Quantidade"><input data-out-chance="${index}" type="number" min="0" max="100" step="1" value="${Math.round(output.chance * 100)}" title="Chance %"><select class="optional-hide" data-out-kind="${index}"><option value="primary" ${output.kind === 'primary' ? 'selected' : ''}>Principal</option><option value="byproduct" ${output.kind === 'byproduct' ? 'selected' : ''}>Subproduto</option></select><button data-out-remove="${index}">×</button></div>`).join('') : '<div class="craft-empty">Adicione pelo menos um resultado principal.</div>'}</div></div></section>`;
  }

  function recipeRequirements() {
    const learnModes: Array<[CraftLearnMode,string]> = [['automatic','Automática'],['quest','Missão'],['item','Item / livro'],['event','Evento']];
    return `<section class="craft-section"><div class="craft-section-head"><strong>Requisitos e aprendizagem</strong></div><div class="craft-section-body"><div class="craft-grid">
      <div class="craft-field"><label>Nível mínimo</label><input id="recipe-min-level" type="number" min="1" value="${recipe.requirements.minLevel ?? ''}"></div>
      <div class="craft-field"><label>Como aprender</label><select id="recipe-learn-mode">${learnModes.map(([value,label]) => `<option value="${value}" ${recipe.requirements.learnMode === value ? 'selected' : ''}>${label}</option>`).join('')}</select></div>
      ${recipe.requirements.learnMode === 'item' ? `<div class="craft-field"><label>Item que ensina</label><select id="recipe-learn-item">${itemOptions(recipe.requirements.learnItemId)}</select></div>` : ''}
      ${recipe.requirements.learnMode === 'quest' ? `<div class="craft-field"><label>Quest key</label><input id="recipe-learn-quest" value="${esc(recipe.requirements.learnQuestId ?? '')}"></div>` : ''}
      ${recipe.requirements.learnMode === 'event' ? `<div class="craft-field"><label>Event key</label><input id="recipe-event" value="${esc(recipe.requirements.eventKey ?? '')}"></div>` : `<div class="craft-field"><label>Evento obrigatório (opcional)</label><input id="recipe-event" value="${esc(recipe.requirements.eventKey ?? '')}"></div>`}
      <div class="craft-field full"><label>Quests concluídas (keys separadas por vírgula)</label><input id="recipe-completed-quests" value="${esc((recipe.requirements.completedQuests ?? []).join(', '))}"></div>
    </div><div><strong style="font-size:9px">Classes permitidas</strong><div class="craft-grid" style="margin-top:8px">${PLAYABLE_CLASSES.map((entry) => `<label class="craft-check"><input data-recipe-class="${entry.id}" type="checkbox" ${(recipe.requirements.classIds ?? []).includes(entry.id) ? 'checked' : ''}> ${esc(entry.name)}</label>`).join('')}</div></div></div></section>`;
  }

  function recipeTest() {
    const primary = recipe.outputs.find((entry) => entry.kind === 'primary');
    return `<section class="craft-section"><div class="craft-section-head"><strong>Recipe Simulator</strong><span>Não altera o personagem</span></div><div class="craft-section-body"><div class="craft-sim">${recipe.ingredients.map((input,index) => `<div class="craft-field"><label>${esc(listItemStudioRecords().find((item) => item.key === input.itemId)?.name ?? input.itemId || 'Ingrediente')} disponível</label><input data-sim-owned="${index}" type="number" min="0" value="10"></div>`).join('') || '<div class="craft-empty">Adicione ingredientes para calcular fabricação máxima.</div>'}<button id="craft-sim-run" class="craft-btn primary" ${!recipe.ingredients.length || !primary ? 'disabled' : ''}>▶ Calcular e fabricar 1 virtual</button><div id="craft-sim-output" class="craft-sim-result">${primary ? `Resultado principal: ${primary.quantity}x ${esc(listItemStudioRecords().find((item) => item.key === primary.itemId)?.name ?? primary.itemId)}` : 'Configure um resultado principal.'}</div></div></div></section>`;
  }

  function stationEditor() {
    return `<section class="craft-section"><div class="craft-section-head"><strong>Tipo de estação</strong><span>Station #${station.numericId}</span></div><div class="craft-section-body"><div class="craft-grid">
      <div class="craft-field"><label>Nome</label><input data-sfield="name" value="${esc(station.name)}"></div>
      <div class="craft-field"><label>Chave interna</label><input data-sfield="key" value="${esc(station.key)}" ${station.source === 'legacy' ? 'readonly' : ''}></div>
      <div class="craft-field"><label>Ícone</label><input data-sfield="icon" value="${esc(station.icon)}"></div>
      <div class="craft-field"><label>Status</label><select id="station-status">${(['draft','published','disabled'] as CraftStudioStatus[]).map((value) => `<option value="${value}" ${station.status === value ? 'selected' : ''}>${STATUS_LABEL[value]}</option>`).join('')}</select></div>
      <div class="craft-field"><label>Prompt de interação</label><input data-sfield="prompt" value="${esc(station.prompt)}"></div>
      <div class="craft-field"><label>Raio de interação</label><input id="station-radius" type="number" min="1" value="${station.interactionRadius}"></div>
      <div class="craft-field full"><label>Categorias permitidas (vírgula)</label><input id="station-categories" value="${esc(station.categories.join(', '))}"></div>
      <div class="craft-field full"><label>Tags</label><input id="station-tags" value="${esc(station.tags.join(', '))}"></div>
    </div><div class="craft-sim-result">Esta estação é uma definição lógica. A posição física será vinculada pelo Map Editor na próxima etapa.</div></div></section>`;
  }
  ...
  function inspectorHtml() {
    ...
    return `<aside class="craft-inspector"><div class="craft-inspector-head"><strong>INSPECTOR</strong><span style="font-size:8px;color:#7898a8">${issues.filter((issue) => issue.severity === 'error').length} erro(s)</span></div><div class="craft-inspector-body">${issues.length ? issues.map((issue) => `<div class="craft-issue ${issue.severity}"><strong>${issue.severity.toUpperCase()}</strong><br>${esc(issue.message)}</div>`).join('') : '<div class="craft-issue info">✓ Conteúdo válido para publicação.</div>'}<div class="craft-section"><div class="craft-section-head"><strong>Dependências</strong></div><div class="craft-section-body" style="font-size:9px;color:#88a6b5">${mode === 'recipes' ? `${recipe.ingredients.length} ingrediente(s)<br>${recipe.outputs.length} resultado(s)<br>Station: ${esc(recipe.stationTypeId || 'nenhuma')}` : `${uses.length} receita(s) usam esta estação${uses.length ? `<br>${uses.slice(0,6).map((entry) => esc(craftRecipeDisplay(entry))).join('<br>')}` : ''}`}</div></div></div></aside>`;
  }

  function render() {
    const title = mode === 'recipes' ? recipe : station;
    ...
    element.innerHTML = `${catalogHtml()}<main class="craft-editor"><div class="craft-titlebar"><span style="font-size:25px">${esc(title.icon)}</span><div><h2>${esc(title.name)}</h2><small style="color:#708f9f">${mode === 'recipes' ? 'Recipe' : 'Station'} #${title.numericId} · ${esc(title.key)}</small></div><div class="spacer"></div><button id="craft-duplicate" class="craft-btn">Duplicar</button><button id="craft-export" class="craft-btn">Exportar</button><button id="craft-import" class="craft-btn">Importar</button><button id="craft-save" class="craft-btn primary">Salvar</button></div>${mode === 'recipes' ? `<nav class="craft-tabs">${RECIPE_TABS.map(([id,label]) => `<button data-recipe-tab="${id}" class="${recipeTab === id ? 'active' : ''}">${label}</button>`).join('')}</nav>` : ''}${body}<div class="craft-footer-actions"><button id="craft-disable" class="craft-btn">${title.status === 'disabled' ? 'Ativar como Draft' : 'Desativar'}</button><button id="craft-delete" class="craft-btn danger" ${title.source === 'legacy' ? 'disabled' : ''}>Excluir custom</button></div></main>${inspectorHtml()}`;
    bind();
  }
  ...
  function bind() {
    ...
    element.querySelector<HTMLButtonElement>('#craft-sim-run')?.addEventListener('click', () => {
      ...
      const max = amounts.length ? Math.min(...amounts.map(({ input,owned }) => Math.floor(owned / Math.max(1, input.quantity)))) : 0;
      const output = element.querySelector<HTMLElement>('#craft-sim-output');
      if (!output) return;
      const primary = recipe.outputs.filter((entry) => entry.kind === 'primary');
      output.textContent = max > 0 ? `Máximo fabricável: ${max}\nFabricação virtual: ${primary.map((entry) => `${entry.quantity}x ${listItemStudioRecords().find((item) => item.key === entry.itemId)?.name ?? entry.itemId}`).join(', ')}\nMateriais marcados “não consumir” permanecem no inventário.` : 'Materiais insuficientes para fabricar 1 unidade.';
    });
    ...
    element.querySelector<HTMLButtonElement>('#craft-save')!.onclick = () => {
      try {
        ...
        refresh(); toast('Conteúdo salvo.'); render();
      } catch (error) { toast(error instanceof Error ? error.message : 'Falha ao salvar.'); }
    };
    element.querySelector<HTMLButtonElement>('#craft-duplicate')!.onclick = () => { if (mode === 'recipes') recipe = duplicateCraftRecipeRecord(recipe); else station = duplicateCraftStationTypeRecord(station); refresh(); toast('Cópia criada como Draft.'); render(); };
    element.querySelector<HTMLButtonElement>('#craft-disable')!.onclick = () => { if (mode === 'recipes') { recipe.status = recipe.status === 'disabled' ? 'draft' : 'disabled'; recipe = saveCraftRecipeRecord(recipe); } else { station.status = station.status === 'disabled' ? 'draft' : 'disabled'; station = saveCraftStationTypeRecord(station); } refresh(); render(); };
    element.querySelector<HTMLButtonElement>('#craft-delete')!.onclick = () => { try { if (mode === 'recipes') deleteCraftRecipeRecord(recipe); else deleteCraftStationTypeRecord(station); refresh(); toast('Conteúdo removido.'); render(); } catch (error) { toast(error instanceof Error ? error.message : 'Falha ao excluir.'); } };
